...
foreach($globalConfig->inputs as $key => $data){
  ...
  if ($handle) {
    ...
    while (($buffer = fgets($handle, 4096)) !== false) {
      ...
      if( $log_datetime ){
        $lineInfos['timestamp'] = $log_datetime->format(DateTime::ISO8601);
        $lineInfos['@timestamp'] = $log_datetime->format(DateTime::ISO8601);
"monitor.php" 106L, 3312C                                                                                                                                                                         1,1           Top
        $lineInfos['timestamp'] = $log_datetime->format(DateTime::ISO8601);
        $lineInfos['@timestamp'] = $log_datetime->format(DateTime::ISO8601);
      }
      else{
        echo "TimeStamp not correct: ".$lineInfos['timestamp']."\n";
      }
      ...
      $lineInfos['mappings'][$data->type] = $mappings;
      ...
      #~ $params['timestamp'] = $log_datetime->format(DateTime::ISO8601);
